...
public class XmlBasicInteractionComplexWrapper implements Complex,FileSourceContext, ExtendedPsiXmlInteraction<ModelledParticipant> {
    ...
    public void assignComplexAc(String accession) {
        XrefUtils.collectAllXrefsHavingDatabaseAndQualifier(this.interaction.getXrefs(), Xref.COMPLEX_PORTAL_MI, Xref.COMPLEX_PORTAL, Xref.COMPLEX_PRIMARY_MI, Xref.COMPLEX_PRIMARY);
        if (accession != null){
            this.interaction.getXrefs().add(new XmlXref(CvTermUtils.createComplexPortalDatabase(), accession, CvTermUtils.createComplexPrimaryQualifier()));
        }
    }
