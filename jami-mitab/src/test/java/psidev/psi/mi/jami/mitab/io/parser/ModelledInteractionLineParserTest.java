...
public class ModelledInteractionLineParserTest {
    ...
    public void test_read_valid_mitab28() throws ParseException {
        ...
        // 1st interaction
        ModelledInteraction binary1 = parser.MitabLine();
        Assert.assertNotNull(binary1);
        Assert.assertFalse(parser.hasFinished());

        Iterator<ModelledParticipant> iterator = binary1.getParticipants().iterator();
        ModelledParticipant participantA = iterator.next();
        ModelledParticipant participantB = iterator.next();
